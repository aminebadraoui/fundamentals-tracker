...
export const SideBar = ({ links, activeLink }) => {
  return (
    <div className="bg-card py-8 w-1/5 border-0 shadow-md shadow-foreground ">
      {links.map((link, index) => {
        const isActive = link.href === usePathname();

        return (
          <div key={index} className="flex flex-col bg-transparent ">
            <Button asChild variant={isActive ? 'active' : 'outline'} className="border-0 bg-transparent text-primary  ">
